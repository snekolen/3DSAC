...
#f = open('data.csv', 'w')
#f.write("Speaker 1 Volume, Speaker 2 Volume, Total Volume (Vector Total), Total Volume (Actual)\n")
...
while (1):
    ...
    if userInput == "V":    #Changes volume
        ...
        #for u in range(1, 19):
        u = int(input("Enter case no: "))
        if u >= 1 and u <= 10:
            s1.updateLocation([-3, 0, 0])
            s2.updateLocation([3, 0, 0])
        elif u >= 12 and u <= 14:
            s1.updateLocation([-4, 0, 0])
            s2.updateLocation([2, 0, 0])
        else:
            s1.updateLocation([-4, 0, 0])
            s2.updateLocation([2, 0, 0])
        ...
        uL = selCase(u)
        print(u, uL)
        cVecs = calcVectors(sList, uL, volume) #Prints out volume vectors
        print(cVecs)
        play(cVecs[0], cVecs[1], volume)
    
        
        #Printing out location info
        #print("\nUser Location: ", userLocation, userLocation)
        #print("Speaker 1 Location: ", s1.getLocation(), "\nSpeaker 2 Location: ", s2.getLocation())
        #print("\nAudio Vectors: ", calcVectors(sList, userLocation, volume))
